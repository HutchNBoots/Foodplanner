...
- **Phase 2 — Assisted basket-fill.** ~~A human-in-the-loop browser flow (e.g. a Claude-in-Chrome-style assistant, or a Playwright script the user runs themselves) that searches Sainsbury's for each shopping-list item and adds it to the basket, but stops before payment for the user to review and check out manually. Keeps a human confirming both the substitutions (out-of-stock items, pack sizes) and the actual purchase.~~ **Corrected (MVP 2, see `REQUIREMENTS.md` and `DECISIONS.md`): Claude in Chrome has no API this app can invoke and no headless mode, so there's no automation to build here — the flow is the user manually pasting the app's shopping-list export into Claude in Chrome themselves. MVP 2's actual scope is just making that export as easy as possible to hand to an agent.**
