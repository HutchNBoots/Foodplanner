...
- **Kids weekly batch-cook-to-freezer** - the system prompt now requires (not just encourages) at least one kids meal to freeze portions whenever the kids track is active that week, framed as a rotating stash rather than a one-off. Reuses the existing freezer inventory machinery, no schema change. See `DECISIONS.md`'s "Kids weekly batch-cook-to-freezer" entry - can't be verified with a live generation in this sandbox (no `ANTHROPIC_API_KEY`), only with content-assertion tests, so worth the operator keeping an eye on a few real weeks to confirm Claude actually follows it.
- **Sign-up journey (multi-household support)** - the app is no longer single-tenant. `/signup` (gated behind the shared `APP_PASSWORD` as an invite code) creates a household account with an auto-generated username (`family1`, `family2`, ...) and a password the household chooses - no email. A short `/onboarding` wizard (household name, kids count, supermarket, budget) then `/welcome` (shows the username - the only place it's shown, since there's no email to send it to) lead into the app. `/login` now takes username + password. **The pre-existing production household keeps working with zero manual migration** - it's backfilled as `family1` with no password, and its first login with the old shared `APP_PASSWORD` transparently upgrades it into a real account (see `DECISIONS.md`'s "Sign-up journey" entry). Password reset is deliberately not built for v1 (no email infrastructure) - reset a tester's password directly in the database if needed, see `DEPLOY.md`'s "Accounts" section.
- **Pantry-staple-aware Claude-in-Chrome shopping handoff (fully shipped)** - the shopping list page's button is now "Copy shopping prompt" instead of "Copy as plain text": it groups items by category (helps the agent avoid bouncing between unrelated searches), flags likely-already-owned pantry staples (honey, oils, spices, stock cubes...) with `[CHECK]` so Claude asks before adding a whole jar for a teaspoon's worth of use, and embeds verification-strategy guidance from a real 78-item/52-minute Claude-in-Chrome session the operator relayed (batch single-quantity adds, single-click-verify multi-unit ones, periodic rather than per-item basket checks). Staple detection is a static keyword heuristic computed at export time (`src/lib/shopping/pantryStaples.ts`), not a Claude-generated field, specifically so it works on any historical week's list, not just future ones. A new collapsible "Reconcile after shopping" section on the same page lets the household paste Claude's `BOUGHT [N] £X.XX`/`SKIPPED [N] - reason` summary back in - it ticks off the matching `shoppingItems.checked` and shows a spend total per category (Protein/Veg & Fruit/Dairy/Bakery/Frozen/Store Cupboard/Staples/Other). Deliberately session-only, not persisted to the database - see `DECISIONS.md`'s "Paste-back reconciliation: build" entry for why, plus a real category-mapping bug (this app's actual aisle names are "Fresh produce"/"Chilled & dairy", not "Fruit & veg"/"Dairy") caught only by manually running the whole flow end to end, not by the unit tests.
- **"My Recipes" epic** - logged in `REQUIREMENTS.md`'s backlog at the operator's request, not yet scoped in detail or built. Rough shape: a personal saved-recipes collection outside the week-by-week History flow. Needs its own requirements pass before building.
- **Delete this meal** - a "Delete" button next to "Swap this meal" on each recipe card removes that meal entirely (two-tap inline confirm, no undo) and re-aggregates the shopping list from what's left. Same batch-cook restriction as swap - a batch-cook source can't be deleted (other days rely on it as leftovers), the button doesn't even render for one. Freezer-inventory portions consumed by a deleted `usesFreezerItem` meal are *not* restored - documented as an accepted limitation, see `DECISIONS.md`'s "Delete this meal" entry.
- **Calendar-based days selection** - the intake form's "which days" question is no longer a fixed Monday-start week with 3 presets (Full week/Weekdays/Mon-Sat). It's now: pick the date the household's order arrives on a real calendar grid (`CalendarDatePicker.tsx`, built from scratch to match the app's design tokens - not the native date input), an optional display-only delivery time, and a free 1-14 day stepper for how long that shop covers (`DayCountStepper.tsx`). `weekStartDate` still means "day 1 of the plan" but is no longer assumed to be a Monday. The family-occasion (Sat/Sun) and kids-track (Mon-Sat) logic needed no changes to support this - it already keys off day-of-week rather than position, including correctly handling a >7-day span that crosses two Saturdays/Sundays (manually verified end to end, see `DECISIONS.md`'s "Calendar-based days selection" entry). Historical weeks still display correctly via a guarded dual-read (old `daysMode` vs. new `numDays`) on the History page.
